...
**Measured: 9.9× lower time-to-first-token on cache hits on a real Android
phone, with output identical to the uncached path.** Details and caveats below —
including what is *not* yet proven.
 
Measured on Android ARM64 and x86-64.
...
| ⚠️ **Thinly sampled** | The on-device ARM64 run uses one sample per request. Re-run with `-repeats 3` before quoting a distribution. |
...
Qwen2.5-0.5B-Instruct Q4_K_M, 123-token shared prefix, 133-token prompts,
**vanilla llama.cpp** — an upstream clone, no patch, no fork.
 
### Android ARM64
 
8 cores, 4 threads, Release build, 40 requests, 1 sample each.
 
| TTFT | mean | p50 | p95 |
|---|---|---|---|
| Cold (no cache) | 4828 ms | 4752 ms | 5297 ms |
| Fragment reuse | 486 ms | 476 ms | 569 ms |
 
**9.9x on cache hits.** 28 hits / 40 requests. Cold 95% CI [4707, 4948] ms.
The warm path decomposes into ~363 ms decoding the 10-token suffix and ~123 ms
restoring the state blob. Fragment size 1.64 MB.
 
### x86-64
 
Windows, MinGW64, 4 threads, 40 requests, 3 timed repeats each.
...
**7.5x on cache hits.** 24 hits / 40 requests. Cold 95% CI [1351, 1439] ms.
 
The speedup is *higher* on ARM64: prefill dominates state restoration more
heavily on a slower CPU, so the ratio grows. State restoration itself costs
79 ms (x86) to 123 ms (ARM64) for a 123-token prefix.
 
Hit rates (70% / 60%) are properties of the synthetic shared-prefix corpus, not
of any real workload. **The citable number is the speedup on hits, with the hit
rate stated beside it.**
 
### Correctness
 
Verified on both platforms, on every hit. No speedup is reported unless both
rows pass.
 
| | ARM64 | x86-64 |
|---|---|---|
| Warm output identical to cold | 28/28 | 24/24 |
| Warm output matches a **no-prefix** generation | 0/28 | 0/24 |
 
The second row is the one that matters. Timing cannot distinguish a working
fragment from an ignored one — both skip the prefix computation, so both are
fast. Only the output separates them.
 
An earlier raw-tensor implementation reported **8.8x** while 14 of 24 hits
reproduced, token for token, the output of a generation with no prefix at all.
That speedup was dropped context, not cache reuse. It was caught by this control
rather than published. See [`attic/`](attic/) for the post-mortem.
 
Reproduce: [`benchmark/real/BENCHMARK.md`](benchmark/real/BENCHMARK.md).
Manifests: `results-arm64/bench-measured-20260710-153223.json`,
`results/bench-measured-20260710-055842.json`.
 
> **Caveats.** The ARM64 run uses one sample per request (`-repeats 1`), so the
> confidence interval on the warm path is wide; the cold interval is tight. The
> reported `process RSS delta` is unreliable — it reads `VmRSS` before and after,
> but the kernel reclaims pages of the mmapped model mid-run — and is not
> presented here.
...
**Thin on-device sampling.** The ARM64 run records one sample per request. The
cold-path confidence interval is tight, the warm-path one is not. Re-run with
`-repeats 3` before quoting p95s.
...
1. Re-run ARM64 with `-repeats 3`; fix or remove the `process RSS delta` metric,
   which reports negative values because the kernel reclaims mmapped model pages.
